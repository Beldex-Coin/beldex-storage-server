...
static block_update parse_swarm_update(const std::string& response_body) {
    ...
    try {
        ...
        for (const auto& mn_json : master_node_states) {
            ...
            const auto& pk_x25519_hex = mn_json.at("pubkey_x25519").get_ref<const std::string&>();
            const auto& pk_ed25519_hex = mn_json.at("pubkey_ed25519").get_ref<const std::string&>();
...
void MasterNode::bootstrap_data() {
    ...
    if (beldex::is_mainnet) {
        seed_nodes.emplace_back(
                "curve://public.beldex.io:29091/eee01f183b2079a529f4ba8933c0f0fcb8053337e003870ef6467a97f2259d73");
        seed_nodes.emplace_back(
                "curve://seed1.beldex.io:29091/37659353131815666979acea91cefa909e3413811a5453c434879b3d7e5b7031");
        seed_nodes.emplace_back(
                "curve://seed2.rpcnode.stream:29091/e06f6b3396b00430ff1ecfaeaa6f68030b5bcd8c3fd2fe813a0a5baa1cb9d008");
        seed_nodes.emplace_back(
                "curve://seed3.beldex.io:29091/bcf02e1364237e549f45accc8cab95895198c0bc0e78a86f52df74d1c2dd8204");
    } else {
        seed_nodes.emplace_back(
                "curve://54.80.140.73:19091/6713a9a96ea47b25223de373edc0203cf8b4d625e96bf2656b042db8e398064");
    }
...
void MasterNode::relay_data_reliable(const std::string& blob, const mn_record& mn) const {
    BELDEX_LOG(debug, "Relaying data to: {} (x25519 pubkey {})", mn.pubkey_legacy, mn.pubkey_x25519);
    ...
    omq_server_->request(
            ...
            [](bool success, auto&& data) {
                if (!success)
                    BELDEX_LOG(err, "Failed to relay batch data: timeout");
            },
            blob);
}
...
OStream& operator<<(OStream& os, const MnodeStatus& status) {
    switch (status) {
        case MnodeStatus::UNSTAKED: return os << "Unstaked";
        case MnodeStatus::DECOMMISSIONED: return os << "Decommissioned";
        case MnodeStatus::ACTIVE: return os << "Active";
...
void MasterNode::on_swarm_update(block_update&& bu) {
    ...
    if (bu.block_hash != block_hash_) {
        BELDEX_LOG(debug, "new block, height: {}, hash: {}", bu.height, bu.block_hash);

        if (bu.height > block_height_ + 1 && block_height_ != 0) {
            BELDEX_LOG(warn, "Skipped some block(s), old: {} new: {}", block_height_, bu.height);
            ...
            BELDEX_LOG(warn, "new block height is not higher than the current height");
        }
    ...
    omq_server_.beldexd_request(
            ...
            [this](bool success, std::vector<std::string> data) {
                ...
                try {
                    ...
                    BELDEX_LOG(err, "Exception caught on swarm update: {}", e.what());
                }
            },
            params.dump());
}
...
std::vector<std::pair<std::string, std::string>> MasterNode::sign_request(
        std::string_view body) const {
    std::vector<std::pair<std::string, std::string>> headers;
    const auto signature =
            generate_signature(hash_data(body), {our_address_.pubkey_legacy, our_seckey_});
    headers.emplace_back(
            http::MNODE_SIGNATURE_HEADER, oxenc::to_base64(util::view_guts(signature)));
    headers.emplace_back(
            http::MNODE_SENDER_HEADER, oxenc::to_base32z(our_address_.pubkey_legacy.view()));
    return headers;
}
...
void MasterNode::process_storage_test_response(
    ...
    if (status.empty()) {
        ...
        // running yet)
        BELDEX_LOG(debug, "Failed to send a storage test request to mnode: {}", testee.pubkey_legacy);
    } else if (status == "OK") {
        if (answer == msg.data) {
            BELDEX_LOG(
                    debug,
...
void MasterNode::report_reachability(const mn_record& mn, bool reachable, int previous_failures) {
    auto cb = [mn_pk = mn.pubkey_legacy, reachable](bool success, std::vector<std::string> data) {
        ...
        try {
            ...
            BELDEX_LOG(err, "Could not report node status: bad json in response");
        }
    };
...
std::pair<MessageTestStatus, std::string> MasterNode::process_storage_test_req(
        uint64_t blk_height, const legacy_pubkey& tester_pk, const std::string& msg_hash_hex) {
    std::lock_guard guard(mn_mutex_);
    ...
    // 2. Check tester/testee pair
    {
        ...
        if (tester.pubkey_legacy != tester_pk) {
            BELDEX_LOG(debug, "Wrong tester: {}, expected: {}", tester_pk, tester.pubkey_legacy);
            return {MessageTestStatus::WRONG_REQ, ""};
        } else {
...
void MasterNode::initiate_peer_test() {
    ...
    if (block_height_ < TEST_BLOCKS_BUFFER) {
        BELDEX_LOG(debug, "Height {} is too small, skipping all tests", block_height_);
        return;
    }
...
std::optional<std::vector<std::string>> MasterNode::update_messages_expiry(
        const user_pubkey_t& pubkey,
...
std::optional<std::vector<std::string>> MasterNode::update_all_expiries(
...
void MasterNode::process_push_batch(const std::string& blob) {
    ...
    BELDEX_LOG(debug, "Got {} messages from peers, size: {}", items.size(), blob.size());
...
std::vector<mn_record> MasterNode::get_swarm_peers() {
    std::lock_guard guard{mn_mutex_};
