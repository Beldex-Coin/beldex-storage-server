...
parse_result parse_cli_args(int argc, char* argv[]) {
    ...
    CLI::App cli{"Beldex Storage Server"};
    cli.formatter(std::make_shared<WrapFormatter>());

    std::filesystem::path data_dir, testnet_dir;
    auto base_dir = util::get_home_dir().value_or(fs::current_path());
    if (!(base_dir == fs::path("/var/lib/beldex") || base_dir == fs::path("/var/lib/beldex")))
        base_dir /= ".beldex";
    if (testnet) {
        base_dir /= "testnet";
    ...
    cli.add_option("--beldexd-rpc", options.beldexd_omq_rpc, "OMQ RPC address on which beldexd is available; typically ipc:///path/to/beldexd.sock or tcp://localhost:22025")->type_name("OMQ_URL")->capture_default_str();
