...
namespace {
    ...
    int mkcert(X509** x509p, EVP_PKEY** pkeyp, int bits, int serial, int days) {
        ...
        X509_NAME_add_entry_by_txt(
                name, "O", MBSTRING_ASC, (const unsigned char*)"Beldex", -1, -1, 0);
...
void generate_dh_pem(const std::filesystem::path& dh_path) {
    ...
    DH* dh = DH_new();
    if (dh == NULL) {
        BELDEX_LOG(err, "Alloc for dh failed");
        ERR_print_errors_fp(stderr);
        abort();
    }
    BELDEX_LOG(info, "Generating DH parameter, this might take a while...");
    ...
    BELDEX_LOG(info, "DH parameter done!");
    FILE* pFile = NULL;
