    ...
    json swarm_to_json(const SwarmInfo& swarm) {
        ...
        for (const auto& mn : swarm.mnodes) {
            mnodes_json.push_back(
                    json{{"address",
                          oxenc::to_base32z(mn.pubkey_legacy.view())
                                  + ".mnode"},  // Deprecated, use pubkey_legacy instead
                         {"pubkey_legacy", mn.pubkey_legacy.hex()},
                         {"pubkey_x25519", mn.pubkey_x25519.hex()},
                         {"pubkey_ed25519", mn.pubkey_ed25519.hex()},
                         {"port",
                          std::to_string(mn.port)},  // Deprecated port (as a string) for backwards
...
RequestHandler::RequestHandler(MasterNode& mn, const ChannelEncryption& ce, ed25519_seckey edsk) :
...
Response RequestHandler::handle_wrong_swarm(const user_pubkey_t& pubKey) {
    ...
    return {http::MISDIRECTED_REQUEST, swarm_to_json(master_node_.get_swarm(pubKey))};
}
...
std::pair<std::shared_ptr<swarm_response>, std::unique_lock<std::mutex>> static setup_recursive_request(
        MasterNode& mn, RPC& req, std::function<void(Response)> cb) {
...
void RequestHandler::process_client_req(rpc::info&&, std::function<void(beldex::Response)> cb) {
    return cb(Response{
            http::OK,
...
void RequestHandler::process_onion_req(RelayToServerInfo&& info, OnionRequestMetadata&& data) {
    BELDEX_LOG(debug, "We are to forward the request to url: {}{}", info.host, info.target);
