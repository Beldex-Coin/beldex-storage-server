...
void OxenmqServer::handle_get_logs(oxenmq::Message& message) {
    ...
    auto dev_sink = dynamic_cast<beldex::dev_sink_mt*>(spdlog::get("beldex_logger")->sinks()[2].get());
        ...
        it->second(
                *request_handler_,
                ...
                [send = message.send_later(),
                 bt_encoded = !params.empty() && params.front() == 'd'](beldex::Response res) {
                    std::string dump;
...
#define LMQ_LOG_MAP(LMQ_LVL, SS_LVL) \
    case oxenmq::LogLevel::LMQ_LVL: BELDEX_LOG(SS_LVL, "[{}:{}]: {}", file, line, message); break;
...
void OxenmqServer::init(
        MasterNode* mn, RequestHandler* rh, RateLimiter* rl, oxenmq::address beldexd_rpc) {
