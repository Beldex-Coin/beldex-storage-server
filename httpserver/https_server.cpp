...
bool HTTPSServer::check_ready(HttpResponse& res) {
    if (std::string reason; !master_node_.mnode_ready(&reason)) {
        ...
        error_response(
                res, http::SERVICE_UNAVAILABLE, "Master node is not ready: " + reason + "\n");
        return false;
    }
    return true;
}
...
static std::variant<legacy_pubkey, Response> validate_mnode_signature(
        MasterNode& mn, const Request& r, bool prevalidate = false) {
            ...
            [this](std::shared_ptr<call_data> data) mutable {
                ...
                omq.inject_task(
                        ...
                        [this, data = std::move(data)]() mutable {
                            ...
                            resp.headers.emplace_back(
                                    http::MNODE_SIGNATURE_HEADER, cert_signature_);

                            legacy_pubkey tester_pk;
                            if (auto it = req.headers.find(http::MNODE_SENDER_HEADER);
                                it != req.headers.end()) {
