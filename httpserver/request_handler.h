...
class RequestHandler {
    ...
    // explicitly
    Response process_mnodes_by_pk(const nlohmann::json& params) const;
  ...
  public:
    RequestHandler(MasterNode& mn, const ChannelEncryption& ce, ed25519_seckey ed_sk);
    ...
    //
    // See beldex-core/rpc/core_rpc_server_command_defs.h for parameters to these endpoints.
    //
    // Returns (via the response callback) the beldexd JSON object on success; on failure returns
    // a failure response with a body of the error string.
    void process_beldexd_request(const nlohmann::json& params, std::function<void(Response)> cb);
