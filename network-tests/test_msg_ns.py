...
def test_store_ns(omq, random_mn, sk, exclude):
    ...
    assert len(spub["swarm"]) == len(swarm['mnodes'])
    ...
    assert len(spriv["swarm"]) == len(swarm['mnodes'])
    edkeys = {x['pubkey_ed25519'] for x in swarm['mnodes']}
...
def test_legacy_closed_ns(omq, random_mn, sk, exclude):
    ...
    assert len(sclosed["swarm"]) == len(swarm['mnodes'])
    edkeys = {x['pubkey_ed25519'] for x in swarm['mnodes']}
...
def test_store_invalid_ns(omq, random_mn, sk, exclude):
    swarm = ss.get_swarm(omq, random_mn, sk)
...
def test_public_outbox(omq, random_mn, sk, exclude):
    swarm = ss.get_swarm(omq, random_mn, sk)
