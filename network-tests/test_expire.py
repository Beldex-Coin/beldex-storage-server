import ss
from util import mn_address, random_time_delta_ms
import time
import base64
import json
...
def test_expire_shorten_extend(omq, random_mn, sk, exclude):
    swarm = ss.get_swarm(omq, random_mn, sk)
    ...
    for mnpk, s in e[0]['swarm'].items():
        ...
        edpk = VerifyKey(mnpk, encoder=HexEncoder)
...
def test_expire_multi(omq, random_mn, sk, exclude):
    swarm = ss.get_swarm(omq, random_mn, sk)
    mns = ss.random_swarm_members(swarm, 2, exclude)
    conns = [omq.connect_remote(mn_address(mn)) for mn in mns]
