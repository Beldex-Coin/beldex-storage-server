...
TEST_CASE("onion request - relay to mnode", "[onion][mnode]") {
    auto data = prefix + R"#({
...
TEST_CASE("onion request - url target filtering", "[onion][relay]") {
    ...
    CHECK_FALSE(is_onion_url_target_allowed("/beldex/v3"));
    CHECK_FALSE(is_onion_url_target_allowed("/beldex/v3/lsrpc?foo=bar"));
}
