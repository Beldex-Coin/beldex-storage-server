...
int usage(std::string_view argv0, std::string_view err = "") {
    ...
    std::cerr
            << "Usage: " << argv0
            << R"( [--mainnet] [--quic] [--xchacha20|--aes-gcm|--aes-cbc|--random] MNODE_PK [MNODE_PK ...] PAYLOAD CONTROL
...
--mainnet makes the onion requests over mainnet master nodes (MNODE_PK values should be mainnet
        ...
        try {
            ...
            for (auto& mn : bns) {
                ...
                if (pk == pubkeys_hex.front()) {
                    ...
                    first_port = mn.at(quic ? "storage_lmq_port" : "storage_port").get<uint16_t>();
                }
            }
            got.set_value();
        }
    ...
    try {
        ...
        for (auto& pk : pubkeys) {
            if (auto it = aux_keys.find(pk); it != aux_keys.end())
                ...
                std::cerr << pk.hex() << " is not an active MN\n";
        }
