...
static constexpr auto ALPN = "beldexstorage"sv;
...
QUIC::QUIC(
    ...
    master_node_->omq_server()->add_category(
            "quic",
...
void QUIC::handle_ping(std::shared_ptr<quic::message> msg) {
    ...
    master_node_->update_last_ping(mnode::ReachType::QUIC);
    msg->respond("pong");
}
...
void QUIC::handle_request(std::shared_ptr<quic::message> msg) {
    ...
    // `mn_mutex_` we could deadlock (because the `open_stream` we do in reachability testing is
    // synchronous, but is also called with the `mn_mutex_` held).
    omq.inject_task(
...
void QUIC::reachability_test(std::shared_ptr<mnode::mn_test> test) {
    ...
    s->command("mnode_ping", ""s, [test = std::move(test), this](const quic::message& m) mutable {
        bool passed;
        ...
        master_node_->omq_server()->inject_task(
