...
class OMQ : public MQBase {
    ...
    ///     be used for pubkeys that are ed keys (but not bd session ids, see the next entry)
    ...
    ///   - 6 -- wrong swarm -- the given pubkey is not stored by this master node's swarm.
    /// - error -- included whenever `errcode` is, this contains an English description of the
    ///   error.
    ///
    /// Each time a message is received the master node sends a message to the connection with a
    ...
    // Connects (and blocks until connected) to beldexd.  When this returns an beldexd connection
    // will be available (and beldexd_conn_ will be set to the connection id to reach it).
  ...
  public:
    ...
    // Encodes the onion request data that we send for internal MN-to-MN onion requests starting
    // at HF18.
