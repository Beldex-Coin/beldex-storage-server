...
static constexpr std::string_view access_denied_ping = "Access denied to mn.ping";

void omq_logger(oxenmq::LogLevel level, const char* file, int line, std::string message) {
    // Downgrade warnings about mn.ping because there are broken nodes out there that don't know
    ...
    switch (level) {
        ...
        case oxenmq::LogLevel::info: 
            log::info(logcat, omq_log_format, file, line, message); 
            break;
