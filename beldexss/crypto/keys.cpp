...
static T parse_pubkey(std::string_view pubkey_in) {
    ...
    if (pubkey_in.size() == 32)
        ...
        oxen::log::warning(logcat, "Invalid public key: not valid bytes, hex, b64, or b32z encoded");
        oxen::log::debug(
                logcat,
...
std::array<unsigned char, 32> subkey_verify_key(
    ...
    // Need to compute: (c + H(c || A, key="BeldexSSSubkey")) A and use that instead of A for
    // verification:

    // H(c || A, key="BeldexSSSubkey"):
