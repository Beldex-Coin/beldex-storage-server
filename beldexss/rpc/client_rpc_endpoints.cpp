    ...
    void load_subaccount(
        ...
        if constexpr (std::is_same_v<json, Dict>) {
            if (oxenc::is_base64(sa) && sa.size() == SUBACCOUNT_TOKEN_LENGTH * 4 / 3)
                ...
                throw parse_error{
                        "invalid subaccount: expected base64 or hex-encoded 32-byte subkey index"};
    ...
    for (const auto& sa : *revoke) {
        if constexpr (std::is_same_v<json, Dict>) {
            if (oxenc::is_base64(sa) && sa.size() == SUBACCOUNT_TOKEN_LENGTH * 4 / 3)
                ...
                throw parse_error{"invalid revoke: expected base64 or hex-encoded 32-byte subkey tag"};
        } else {
            if (sa.size() != SUBACCOUNT_TOKEN_LENGTH)
...
void beldexd_request::load_from(json params) {
    load(*this, params);
}
void beldexd_request::load_from(bt_dict_consumer params) {
    load(*this, params);
}
...
static void load_condition(ifelse& i, Dict if_) {
    ...
    i.condition = [=](const mnode::MasterNode& mnode) {
        bool result = true;
        if (hf_ge || hf_lt) {
            std::array<int, 2> hf = {mnode.hf().first, mnode.hf().second};
            if (hf_ge)
                result &= hf >= *hf_ge;
            if (hf_lt)
                result &= hf < *hf_lt;
        ...
        if (height_ge || height_lt) {
            auto height = static_cast<int>(mnode.blockheight());
            if (height_ge)
