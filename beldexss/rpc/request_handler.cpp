...
namespace {
    json swarm_to_json(const std::optional<mnode::SwarmInfo>& swarm) {
        if (!swarm)
            return json{
                    {"mnodes", json::array()},
                    {"swarm", util::int_to_string(mnode::INVALID_SWARM_ID, 16)},
            };
        json mnodes_json = json::array();
        for (const auto& mn : swarm->mnodes) {
            mnodes_json.push_back(
                    json{{"address",  // Deprecated, use pubkey_legacy instead
                          oxenc::to_base32z(mn.pubkey_legacy.view()) + ".mnode"},
    ...
    bool verify_signature(
        ...
        if ((pubkey.type() == 189 || (pubkey.type() == 0 && !is_mainnet)) && pk_ed25519) {
            pk = pk_ed25519->data();
        ...
        if (subkey) {
            ...
            // Compute our verification key: (c + H("BeldexSSSubkey" || c || A)) A and use that
...
Response RequestHandler::handle_wrong_swarm(const user_pubkey_t& pubKey) {
    ...
    json swarm = swarm_to_json(master_node_.get_swarm(pubKey));
    add_misc_response_fields(swarm, master_node_);
...
std::pair<std::shared_ptr<swarm_response>, std::unique_lock<std::mutex>> static setup_recursive_request(
        mnode::MasterNode& mn, RPC& req, std::function<void(Response)> cb) {
...
void RequestHandler::process_client_req(rpc::store&& req, std::function<void(Response)> cb) {
    ...
    auto max_ttl = (!public_ns && master_node_.hf_at_least(mnode::HARDFORK_EXTENDED_PRIVATE_TTL))
                         ? TTL_MAXIMUM_PRIVATE
                         : TTL_MAXIMUM;
    ...
    std::string message_hash =
            master_node_.hf_at_least(mnode::HARDFORK_HASH_NO_TIME)
    ...
    bool success = false;
    try {
        success = master_node_.process_store(
                message{req.pubkey,
                        message_hash,
                        req.msg_namespace,
                        req.timestamp,
    ...
    if (entry_router) {
        ...
        add_misc_response_fields(res->result, master_node_, now);
    }
...
void RequestHandler::process_client_req(
        rpc::beldexd_request&& req, std::function<void(rpc::Response)> cb) {
...
void RequestHandler::process_client_req(
    ...
    log::debug(
            ...
            obfuscate_pubkey(req.pubkey),
            swarm ? swarm->mnodes.size() : 0);

    auto body = swarm_to_json(swarm);
    add_misc_response_fields(body, master_node_);
...
void RequestHandler::process_client_req(
    ...
    bool more = false;
    try {
        std::tie(msgs, more) = master_node_.get_db().retrieve(
                req.pubkey,
                req.msg_namespace,
                req.last_hash.value_or(""),
                req.max_count,
                req.max_size);
        master_node_.record_retrieve_request();
    } catch (const std::exception& e) {
        auto msg = fmt::format(
...
void RequestHandler::process_client_req(
    ...
    if (is_all(req.msg_namespace)) {
        handle_action_all_ns(
                mine,
                "deleted",
                master_node_.get_db().delete_all(req.pubkey),
                req.b64,
                ed25519_sk_,
        ...
        handle_action_one_ns(
                mine,
                "deleted",
                master_node_.get_db().delete_all(
...
void RequestHandler::process_client_req(rpc::delete_msgs&& req, std::function<void(Response)> cb) {
    ...
    auto deleted = master_node_.get_db().delete_by_hash(req.pubkey, req.messages);
    ...
    if (req.recurse)
        add_misc_response_fields(res->result, master_node_);
...
void RequestHandler::process_client_req(
    ...
    master_node_.get_db().revoke_subkey(req.pubkey, req.revoke_subkey);
...
void RequestHandler::process_client_req(
    ...
    if (is_all(req.msg_namespace)) {
        handle_action_all_ns(
                mine,
                "deleted",
                master_node_.get_db().delete_by_timestamp(req.pubkey, req.before),
                req.b64,
                ed25519_sk_,
                req.pubkey.prefixed_hex(),
                req.before);
        ...
        handle_action_one_ns(
                mine,
                "deleted",
                master_node_.get_db().delete_by_timestamp(
...
void RequestHandler::process_client_req(rpc::expire_all&& req, std::function<void(Response)> cb) {
    ...
    if (is_all(req.msg_namespace)) {
        handle_action_all_ns(
                mine,
                "updated",
                master_node_.get_db().update_all_expiries(req.pubkey, req.expiry),
                req.b64,
                ed25519_sk_,
                req.pubkey.prefixed_hex(),
                req.expiry);
        ...
        handle_action_one_ns(
                mine,
                "updated",
                master_node_.get_db().update_all_expiries(
...
void RequestHandler::process_client_req(rpc::expire_msgs&& req, std::function<void(Response)> cb) {
    ...
    // TODO: remove after HF19.3
    if (!master_node_.hf_at_least(mnode::HARDFORK_EXPIRY_SHORTEN_ONLY) &&
        (req.shorten || req.extend))
        return cb(Response{
                http::BAD_REQUEST,
                "expire: shorten/extend parameters cannot be used before network version {}.{}"_format(
                        mnode::HARDFORK_EXPIRY_SHORTEN_ONLY.first,
                        mnode::HARDFORK_EXPIRY_SHORTEN_ONLY.second)});
    ...
    auto max_ttl = master_node_.hf_at_least(mnode::HARDFORK_EXTENDED_PRIVATE_TTL)
                         ? TTL_MAXIMUM_PRIVATE
                         : TTL_MAXIMUM;
    auto expiry = std::min(std::chrono::system_clock::now() + max_ttl, req.expiry);
    auto updated = master_node_.get_db().update_expiry(
            req.pubkey,
            req.messages,
            expiry,
    ...
    if (req.extend || req.shorten) {
        ...
        if (!unchanged_hashes.empty())
            unchanged = master_node_.get_db().get_expiries(req.pubkey, unchanged_hashes);
    }
...
void RequestHandler::process_client_req(rpc::get_expiries&& req, std::function<void(Response)> cb) {
    ...
    res["expiries"] = master_node_.get_db().get_expiries(req.pubkey, req.messages);
    return cb(Response{http::OK, std::move(res)});
}
...
void RequestHandler::process_client_req(rpc::ifelse&& req, std::function<void(rpc::Response)> cb) {
    ...
    bool cond = req.condition(master_node_);
    json response{
            {"hf", master_node_.hf()},
            {"v", STORAGE_SERVER_VERSION},
            {"height", master_node_.blockheight()},
            {"condition", cond}};
...
Response RequestHandler::process_retrieve_all() {
    std::vector<message> msgs;
    try {
        msgs = master_node_.get_db().retrieve_all();
    } catch (const std::exception& e) {
...
void RequestHandler::process_onion_req(FinalDestinationInfo&& info, OnionRequestMetadata&& data) {
    ...
    if (!master_node_.mnode_ready())
        return data.cb(wrap_proxy_response(
                {http::SERVICE_UNAVAILABLE, "mnode not ready"s},
                data.ephem_key,
                data.enc_type,
                info.json,
                info.base64));
...
void RequestHandler::process_onion_req(RelayToNodeInfo&& info, OnionRequestMetadata&& data) {
    ...
    log::debug(logcat, "send_onion_to_mn, mn: {}", dest_node->pubkey_legacy);
